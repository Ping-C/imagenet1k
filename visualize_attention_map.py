...
if __name__ == "__main__":
    ...
    # get data
    images_all, labels = get_images(args.data_path, num_images=args.num_images)


    plot_limit = 20
    num_images= args.num_images
    fig, axes = plt.subplots(min(num_images, plot_limit)+1, 2*len(args.model_paths), figsize=(2*4*len(args.model_paths), (min(num_images, plot_limit)+1)*4))
    # load model
    ...
    for model_i, model_path in enumerate(args.model_paths):
        ...
        for img_i in range(num_images):
            ...
            cum_att = cum_att[0].mean(dim=0).reshape(14, 14).detach()
            average_att.append(cum_att[None])
            if img_i < plot_limit:
                axes[img_i+1][model_i*2].imshow(images[0].permute(1, 2, 0).cpu()/2+0.5)
                axes[img_i+1][model_i*2+1].imshow(cum_att)
        average_att = ch.cat(average_att, dim=0).mean(dim=0)
        axes[0][model_i*2+1].imshow(average_att)
