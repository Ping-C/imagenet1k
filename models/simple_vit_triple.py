...
class LayerNormTriple(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.layernorm_clean = nn.LayerNorm(dim)
        self.layernorm_adv0 = nn.LayerNorm(dim)
        self.layernorm_adv1 = nn.LayerNorm(dim)
        self.clean = True
        self.adv_idx = 0
    def forward(self, x):
        if self.clean:
            ...
            if self.adv_idx==0:
                return self.layernorm_adv0(x)
            elif self.adv_idx==1:
                return self.layernorm_adv1(x)
    def make_clean(self):
        self.clean = True
    def make_adv(self, idx=0):
        self.clean = False
        self.adv_idx = idx
...
class SimpleViTTripleLN(nn.Module):
    ...
    def make_clean(self):
        for module in self.modules():
            if isinstance(module, LayerNormTriple):
                module.make_clean()
    def make_adv(self, idx=0):
        for module in self.modules():
            if isinstance(module, LayerNormTriple):
                module.make_adv(idx=idx)
