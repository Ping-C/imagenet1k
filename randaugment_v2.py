...
class RandAugmentV4(Operation):
        ...
        def randaug(im, mem):
            ...
            for i in my_range(im.shape[0]):
                for n in range(num_ops):
                    ...
                    idx = np.random.randint(low=1, high=16+1)
                    mag = magnitudes[idx-1]
                    if np.random.random() < 0.5:
                        mag = mag * is_signed[idx-1] 
    ...
    def declare_state_and_memory(self, previous_state: State) -> Tuple[State, Optional[AllocationQuery]]:
        ...
        return replace(previous_state, shape=previous_state.shape), [
            AllocationQuery(previous_state.shape, dtype=np.dtype('uint8')), 
            AllocationQuery((1, h, w, c), dtype=np.dtype('uint8')),
            AllocationQuery((c, 256), dtype=np.dtype('int32')),
            AllocationQuery((1, h, w, c), dtype=np.dtype('float32')),
        ]
...
def cutout(source, destination, crop_size):
    ...
    destination[:]=source
    destination[np.maximum(coord[0]-crop_size, 0):np.minimum(coord[0]+crop_size, source.shape[0]-1), np.maximum(coord[1]-crop_size, 0):np.minimum(coord[1] + crop_size, source.shape[1]-1)] = np.array((128,128,128))
